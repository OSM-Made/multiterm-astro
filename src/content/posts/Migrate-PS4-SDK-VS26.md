...
.DESCRIPTION
    ...
    DLL (after backing up the original) and runs the devenv /setup command.
...
.NOTES
    - This script must be run with administrator privileges due to copying files into "Program Files"
      and running the 'devenv /setup' command.
    - Ensure Visual Studio 2026 is closed before running the script.
    - A backup of the original v180 DLL will be created as .backup if it doesn't already exist.
#>
param(
...
# --- STEP 5: Backup and Copy Required DLL (v170 to v180) ---
...
$DllPath = "$Dest5\Microsoft.Build.CPPTasks.Common.Base.dll"
$BackupPath = "$DllPath.backup"

# Backup the original v180 DLL if it exists and backup doesn't already exist
if (Test-Path $DllPath) {
    if (Test-Path $BackupPath) {
        Write-Host "-> Backup already exists at $BackupPath. Skipping backup..." -ForegroundColor Yellow
    }
    else {
        Write-Host "-> Backing up original v180 DLL..." -ForegroundColor Cyan
        try {
            Rename-Item -Path $DllPath -NewName "$DllPath.backup" -Force
            Write-Host "   SUCCESS: Backup created at $BackupPath" -ForegroundColor Green
        }
        catch {
            Write-Host "   ERROR: Failed to backup DLL. Details: $($_.Exception.Message)" -ForegroundColor Red
            exit 1
        }
    }
}
else {
    Write-Host "-> No existing v180 DLL found to backup. Proceeding with copy..." -ForegroundColor Yellow
}
